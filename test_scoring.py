...
class TestScoring(unittest.TestCase):
    def test_perfect_score(self):
        results = [
            CheckResult(
                check_id=f"check_{i}", check_name=f"Test {i}",
                category="Technical SEO", severity=Severity.HIGH,
                passed=True, score=100, detail="",
            )
            for i in range(5)
        ]
    ...
    def test_mixed_results(self):
        results = [
            CheckResult(
                check_id=f"check_{i}", check_name=f"Test {i}",
                category="Technical SEO", severity=Severity.HIGH,
                passed=(i % 2 == 0), score=100 if i % 2 == 0 else 0, detail="",
            )
            for i in range(10)
        ]
        score = compute_score(results)
